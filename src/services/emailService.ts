// Serviço de e-mail usando SendGrid
// Nota: Em produção, esta API key deve estar em variáveis de ambiente do backend

const SENDGRID_API_KEY = import.meta.env.VITE_SENDGRID_API_KEY;
const FROM_EMAIL = import.meta.env.VITE_FROM_EMAIL || 'noreply@seuapp.com';
...
export const sendInviteEmail = async (inviteData: InviteData): Promise<{ success: boolean; error?: string }> => {
  try {
    // Como estamos no frontend, vamos usar uma abordagem diferente
    // Vamos criar um template de e-mail que pode ser enviado via API
    ...
    // Para usar SendGrid no frontend, você precisará de uma API backend
    // Por enquanto, vamos simular o envio e mostrar o conteúdo do e-mail
    
    console.log('E-mail que seria enviado:', {
      to: inviteData.email,
...
// Função para enviar e-mail usando uma API backend (recomendado)
export const sendInviteEmailViaAPI = async (inviteData: InviteData): Promise<{ success: boolean; error?: string }> => {
  try {
    // Esta função seria chamada para uma API backend que usa SendGrid
